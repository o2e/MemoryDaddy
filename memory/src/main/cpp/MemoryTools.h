...
PMAPS readmaps_all(int pid);//读取maps文件
PMAPS readmaps_bad(int pid);//读取maps文件
PMAPS readmaps_c_alloc(int pid);//读取maps文件
PMAPS readmaps_c_bss(int pid);//读取maps文件
PMAPS readmaps_code_app(int pid);//读取maps文件
PMAPS readmaps_c_data(int pid);//读取maps文件
PMAPS readmaps_c_heap(int pid);//读取maps文件
PMAPS readmaps_java_heap(int pid);//读取maps文件
PMAPS readmaps_a_anonmyous(int pid);//读取maps文件
PMAPS readmaps_code_system(int pid);//读取maps文件
PMAPS readmaps_stack(int pid);//读取maps文件
PMAPS readmaps_ashmem(int pid);//读取maps文件
...
MAPS * MemorySearch(int pid, int range, char *value, int *gs, TYPE type);
void MemorySearch(char *bm, char *value, int *gs, int TYPE);//类型搜索,这里value需要传入一个地址
PMAPS MemorySearch_DWORD(int pid, int value, int *gs, PMAPS pMap);  //内存搜索DWORD
PMAPS MemorySearch_FLOAT(int pid, float value, int *gs, PMAPS pMap);  //内存搜索FLOAT
...
void RangeMemoryOffset(char *bm, char *from_value, char *to_value, long int offset, int *gs,
                       int type);//范围偏移
PMAPS RangeMemoryOffset_DWORD(char *bm, int from_value, int to_value, long int offset, PMAPS pBuff,
                              int *gs);//搜索偏移DWORD
PMAPS
RangeMemoryOffset_FLOAT(char *bm, float from_value, float to_value, long int offset, PMAPS pBuff,
                        int *gs);//搜索偏移FLOAT
...
int killProcessByOs(char *bm);//杀掉进程
...
PMAPS
BaseAddressSearch_FLOAT(PACKAGENAME *bm, float value, COUNT *gs, ADDRESS BaseAddr, PMAPS pMap) {
    *gs = 0;
    pid_t pid = getPID(bm);
    if (pid == 0) {
        puts("can not get pid");
...
void
RangeMemorySearch(PACKAGENAME *bm, char *from_value, char *to_value, COUNT *gs, TYPE type)//范围搜索
{
    PMAPS pHead = NULL;
    PMAPS pMap = NULL;
    switch (MemorySearchRange) {
...
PMAPS
RangeMemorySearch_DWORD(PACKAGENAME *bm, int from_value, int to_value, COUNT *gs, PMAPS pMap)//DWORD
{
    pid_t pid = getPID(bm);
    if (pid == 0) {
        puts("can not get pid");
...
PMAPS RangeMemorySearch_FLOAT(PACKAGENAME *bm, float from_value, float to_value, COUNT *gs,
                              PMAPS pMap)//FLOAT
{
    pid_t pid = getPID(bm);
    if (pid == 0) {
...
PMAPS MemorySearch(int pid, int range, char *value, int *gs, TYPE type) {
    PMAPS pHead = NULL;
    PMAPS pMap = NULL;
    switch (range) {
        case ALL:
...
void RangeMemoryOffset(PACKAGENAME *bm, char *from_value, char *to_value, OFFSET offset, COUNT *gs,
                       TYPE type)//范围偏移
{
    PMAPS pHead = NULL;
    switch (type) {
        case DWORD:
            if (atoi(from_value) > atoi(to_value))
                pHead = RangeMemoryOffset_DWORD(bm, atoi(to_value), atoi(from_value), offset, Res,
                                                gs);
            else
                pHead = RangeMemoryOffset_DWORD(bm, atoi(from_value), atoi(to_value), offset, Res,
                                                gs);
            break;
        case FLOAT:
            if (atof(from_value) > atof(to_value))
                pHead = RangeMemoryOffset_FLOAT(bm, atof(to_value), atof(from_value), offset, Res,
                                                gs);
            else
                pHead = RangeMemoryOffset_FLOAT(bm, atof(from_value), atof(to_value), offset, Res,
                                                gs);
            break;
        default:
            printf("\033[32;1mYou Select A NULL Type!\n");
...
PMAPS
RangeMemoryOffset_DWORD(PACKAGENAME *bm, int from_value, int to_value, OFFSET offset, PMAPS pBuff,
                        COUNT *gs)//搜索偏移DWORD
{
    pid_t pid = getPID(bm);
    if (pid == 0) {
...
PMAPS RangeMemoryOffset_FLOAT(PACKAGENAME *bm, float from_value, float to_value, OFFSET offset,
                              PMAPS pBuff, COUNT *gs)//搜索偏移FLOAT
{
    pid_t pid = getPID(bm);
    if (pid == 0) {
        puts("can not get pid");
...
int killProcessByOs(PACKAGENAME *bm) { // 通过系统函数杀死进程
    int pid = getPID(bm);
    if (pid == 0) {
        return -1;
    }
